...
module.exports = {
	async run(m, args) {
		...
		const previousMessage = await db.get(`${m.author.id}-hiyoPrevMsg`);

		m.channel.messages.fetch(previousMessage).then((msg) => msg.edit(edit)); //edit the message into whatever the user wanted

		let previousContext = await db.get(`${m.author.id}-Hiyohis`);
		if (!previousContext) {
			//go awawy
		...
		lines[lines.length - 1] = `Hiyori: ${edit}\n`; //replace the last line with the edit

		let replies = await db.get(`${m.author.id}-HiyoReplies`);
		replies.push(edit); //add the edit to the replies array (so you can undo it, or cycle through regens)
		db.set(`${m.author.id}-Hiyohis`, lines); //update history
		db.set(`${m.author.id}-HiyoCurrRep`, edit); //mark the new edit as the currently displayed reply
		db.set(`${m.author.id}-HiyoReplies`, replies); //update replies
